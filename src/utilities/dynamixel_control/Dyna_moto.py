...
# Use the bundled Dynamixel_sdk package inside this package via a
# package-relative import. This makes imports work when the project is run
# as a package (python -m ...) and avoids requiring the SDK to be installed
# as a top-level package.
from .Dynamixel_sdk import *                    # Uses bundled Dynamixel SDK
...
'''
These A and b are calculated from the heavier motors performance graph found in: https://emanual.robotis.com/docs/en/dxl/x/xm540-w270/
'''
A = 0.461 #NOTE technically the graph is a negative slope.
B = -0.0956
...
BAUDRATE = 1_000_000#4000000#57600#
class Dyna_moto():
	def __init__(self, baudrate = BAUDRATE, devicename = '/dev/ttyUSB0', protocolversion = 2.0, con_mode = POS_CONTROL, ro = False):#devicename = '/dev/ttyUSB0'
	...
	def get_motor_ids(self):
		print("getmotorid stuff")
		motor_ids = []
		for i in range(self.ping_num):
			dxl_model_number, dxl_comm_result, dxl_error = self.pack_hand.ping(self.port_hand, i)
			print("modelnumber", dxl_model_number, "comm_res:",dxl_comm_result, "error:", dxl_error)
			print(f"Com-Success: {COMM_SUCCESS}")
			if dxl_comm_result != COMM_SUCCESS:
				n=0
			elif dxl_error != 0:
				n=0
